...
MOTION_LIB_PATH = os.environ.get(
    'MOTION_LIB_PATH',
    '/home/ro/lab/simulation/motion-planners')
...
def pbpq2pq(pxyzw):     # converts pybullet position,quaternion to open3d position,quaternion
    pxyzw[3], pxyzw[4], pxyzw[5], pxyzw[6] = pxyzw[6], pxyzw[3], pxyzw[4], pxyzw[5]
    return pxyzw

def visualization(bc, xyzw):
    #works correct in trajectory form of vec7, [p, x, y, z, w]
    scale = 0.05
    xyz = pbpq2pq(xyzw)

    xyz_temp = transform_from_pq(xyz)
    x_line = bc.addUserDebugLine(lineFromXYZ=xyz_temp[:3, 3],
                                            lineToXYZ=xyz_temp[:3, 3] + xyz_temp[:3, 0] * scale,
                                            lineColorRGB=[1, 0, 0], lineWidth=1)
    y_line = bc.addUserDebugLine(lineFromXYZ=xyz_temp[:3, 3],
                                            lineToXYZ=xyz_temp[:3, 3] + xyz_temp[:3, 1] * scale,
                                            lineColorRGB=[0, 1, 0], lineWidth=1)
    z_line = bc.addUserDebugLine(lineFromXYZ=xyz_temp[:3, 3],
