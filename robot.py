...
class UR5RG2(object):
    def __init__(self, bc: BulletClient, pos: TranslationT, orn: QuaternionT):
        self.bc = bc
        self.robot_id: int = bc.loadURDF('ur5_rg2/ur5_rg2.urdf', pos, orn,
        ...
        for i in range(len(self.gripper_link_ids)):
            ...
            # Initially set the gripper as open
    ...
    def close_gripper(self):
        ...
        # position control with max torque limits, tries position control until any joint on gripper reaches limit set

        self.bc.setJointMotorControlArray(self.robot_id, self.gripper_link_ids, self.bc.POSITION_CONTROL,
                                          targetPositions=[i * self.gripper_link_limit[0] for i in
                                                           self.gripper_link_sign],
                                          positionGains=[1 for i in range(len(self.gripper_link_ids))],
                                          forces = [5 for i in range(len(self.gripper_link_ids))])
